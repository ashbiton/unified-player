...
* start with updating the `package.json` with the correct path to your sdk or choose a version.
* Run `npm ci`.
...
* upload:
    ...
    2. push `dist` folder to aws s3 with your bucket name

        ```aws s3 cp dist s3://hyperscale-webui/<your-bucket> --recursive --cache-control no-store```
        replace with your bucket name.

    3. your webui will be exposed in `https://hyperscale-webui.s3.eu-west-1.amazonaws.com/<your bucket>/index.html`